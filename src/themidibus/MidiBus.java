 ...
 * <br>
 * <h2>Typical Implementation, Simple</h2>
 * <br>
 * A typical simple Processing MIDI application would begin by invoking the static method {@link #list()} to learn what devices are available. Then using that information a new MidiBus object would be instantiated with with the desired MIDI input and/or output devices. The Processing sketch could then send midi via MidiBus's outgoing methods such as {@link #sendNoteOn(int, int, int)}, {@link #sendNoteOff(int, int, int)} and {@link #sendControllerChange(int, int, int)} and receive midi via the PApplet methods this package provides support for such as {@link themidibus.PApplet#noteOn(int, int, int)}, {@link themidibus.PApplet#noteOff(int, int, int)} and {@link themidibus.PApplet#controllerChange(int, int, int)}.
 * <h2>Typical Implementation, Advanced</h2>
 * <br>
